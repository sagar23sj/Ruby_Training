=begin
class Employee
        attr_accessor :salary,:no_of_proj,:extra_curri
        attr_reader :name

        def initialize(name,salary,no_of_proj,extra_curri)
                @name = name
                @salary = salary
                @no_of_proj = no_of_proj
                @extra_curri = extra_curri
        end

        def salary_hike()
                if (extra_curri == true)
                        @salary += (@salary*(@no_of_proj*0.1+0.1))
                else
                        @salary += @salary*@no_of_proj*0.1
                end
        end

end

sagar=Employee.new('Sagar Sonwane',10000,3,false)
puts sagar.salary
sagar.salary_hike()
puts sagar.salary

puts "After Changing Number of Prjects and Extra Activities"

sagar.no_of_proj=22
sagar.extra_curri=true
puts sagar.salary
sagar.salary_hike()
puts sagar.salary()
